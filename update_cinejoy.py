...
async def main():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # Strict extraction: Only collect actual movie/show anchor paths
            for a in soup.find_all('a', href=True):
                ...
                # Ensure valid title and media path format
            ...
            # ONLY overwrite the file if actual stream links were parsed (ignoring raw code)
            if len(scraped_items) > 0:
                with open("my_cinema.m3u", "w", encoding="utf-8") as f:
                    f.writelines(playlist_lines)
                print(f"SUCCESS: Extracted {len(scraped_items)} clean entries.")
            else:
                print("WARNING: Captured HTML code instead of media links. Aborting file write to protect playlist.")
